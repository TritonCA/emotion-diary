  ...
  Future<void> _addEmotion(EmotionCategory cat) async {
    ...
    final name = await showDialog<String>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text('Add to ${cat.name}', style: AppTypography.headlineMd(c.onSurface)),
        content: TextField(
          controller: controller,
          autofocus: true,
          style: AppTypography.bodyMd(c.onSurface),
          decoration: const InputDecoration(hintText: 'Emotion name'),
        ),
        actions: [
          TextButton(
          ...
          TextButton(
              ...
              child: Text('Add', style: AppTypography.labelSm(c.primary))),
        ],
      ),
    );
                  ...
                  Theme(
                    ...
                    child: ExpansionTile(
                      leading: Icon(EmotionIcons.forCategory(cat.iconName),
                          color: c.primary),
                      title: Text(cat.name, style: AppTypography.bodyLg(c.onSurface)),
                          ...
                          children: [
                            for (final e in cat.emotions)
                              Container(
                                ...
                                child: Text(e, style: AppTypography.labelSm(c.onSurface)),
                              ),
                            ActionChip(
                              avatar: Icon(Icons.add, size: 16, color: c.primary),
                              label: Text('Add', style: AppTypography.labelSm(c.primary)),
                              backgroundColor: c.surface,
